...
const inputClass =
  "rounded-lg border border-[var(--border-soft)] bg-white/15 px-4 py-3 text-sm text-[var(--text-primary)] outline-none transition focus:border-brand-500 focus:bg-white/15 focus:ring-4 focus:ring-[var(--focus-ring)]";

const BlogQueryPanel = ({
  formValues,
  hasFilters,
  onChange,
  onReset,
    ...
    <div className="surface-panel rounded-xl p-5 sm:p-6">
      <div className="flex flex-col gap-4 lg:flex-row lg:items-end lg:justify-between">
        <div>
          ...
          <h2 className="mt-2 text-3xl font-bold text-[var(--text-primary)] sm:text-4xl">
            Read technical writing from builders
          </h2>
          <p className="mt-3 max-w-2xl text-sm leading-6 text-[var(--text-secondary)]">
            Search published posts by title, tag, category, and popularity.
          </p>
        </div>
      ...
      >
        <label className="relative block">
          ...
          />
          <input
            className="w-full rounded-lg border border-[var(--border-soft)] bg-white/15 py-3 pl-10 pr-4 text-sm text-[var(--text-primary)] outline-none transition focus:border-brand-500 focus:bg-white/15 focus:ring-4 focus:ring-[var(--focus-ring)]"
            name="search"
            onChange={onChange}
